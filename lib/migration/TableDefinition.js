...
class TableDefinition {
	...
	static getSqlValue (value) {
		if (value === null || ['boolean', 'number'].indexOf(typeof value) !== -1 ) {
			return value
		} else {
			return `'${value}'`
		}
  ...
  dropDatabase (dbName, options) {
	...
	_handleFieldColumn (field, type, options) {
		...
		if (type) {
			let columnSqls = []
			if (type === 'string') {
				columnSqls.push(`\`${field}\` VARCHAR(${options['limit'] || 255})`)
			} else if (type === 'text') {
	...
	references (model, options) {
		options = Object.assign({foreignKey: true}, options)
