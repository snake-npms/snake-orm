...
let snakeOrms = {}
let firstSnakeOrm = null
class SnakeOrm {
	static get SnakeModel () {
		return createSnakeModel(SnakeOrm)
	}
	
	static async initWithSnakeOrmRcFile (rcFilePath, beforeDoAfn) {
    ...
    beforeDoAfn && await beforeDoAfn(ormProxy)
		// --- auto run migration
	...
	static getOrmProxyByDatabase (database) {
		if (!database && firstSnakeOrm) {
			return firstSnakeOrm
	...
	async registerModelsWithPath (modelsPath, skipFiles = []) {
		...
		for (let key of Object.keys(this._Models)) {
			await this.dialect.pragmaTableInfo(this._Models[key]['table'])
