...
class SnakeOrm {
	...
	constructor (database, username, password, otherConnectOptions) {
		...
		this._connectOptions = Object.assign({database, username, password, dialect: 'sqlite3'}, otherConnectOptions)
		this.dialect = new Proxy(new DialectProxy(this._connectOptions), {
			get (target, prop) {
				if (target[prop]) {
					return target[prop];
				} else {
