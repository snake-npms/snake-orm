...
class Base {
    ...
    return await this.runSql(`PRAGMA table_info('${tableName}')`).then(result => {
      ...
      result.forEach(item => {
        ...
        defaultValues[item['name']] = item['dflt_value']
        schema[item['name']] = {
          type: item['type'],
          null: item['notnull'] ? false : true,
          isPk: item['pk'] ? true : false,
			...
			return new Promise(async (resolve, reject) => {
				db.serialize(async () => {
					return await this.ns.run(async () => {
						...
						let transactionId = Date.current().strftime('yMd-H:m:s.L') + Math.random().toString(36)
