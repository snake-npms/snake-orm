...
class Action extends BridgeReflection {
	...
	static hasMany (collection, options) {
		...
		if (typeof collection === 'string') {
			CollectionModel = this._snakeOrmProxy._Models[pluralize.singular(collection).toCapitalize()]
		}
		options = Object.assign({foreignKey: `${this.name.toVarCase()}Id`}, options)
		this.prototype[`get${collection.toCapitalize()}`] = async function () {
			let primaryKey = await this.constructor.getPrimaryKey()
