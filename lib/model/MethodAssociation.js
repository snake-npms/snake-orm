...
class MethodAssociation extends Method {
    ...
    let beforeSave = [], beforeCreate = [], afterCreate = [], beforeUpdate = [], afterUpdate = [], afterSave = []
		Object.defineProperties(this, {
      beforeSave: {
        get () { return beforeSave },
        set (nv) { beforeSave = nv }
      },
      beforeCreate: {
        get () { return beforeCreate },
        set (nv) { beforeCreate = nv }
      },
      afterCreate: {
        get () { return afterCreate },
        set (nv) { afterCreate = nv }
      },
      beforeUpdate: {
        get () { return beforeUpdate },
        set (nv) { beforeUpdate = nv }
      },
      afterUpdate: {
        get () { return afterUpdate },
        set (nv) { afterUpdate = nv }
      },
      afterSave: {
        get () { return afterSave },
        set (nv) { afterSave = nv }
      },
			__association: {
				value: {},
	...
	static hasMany (collection, options) {
        ...
        if (hasHas) {
          ...
          return ModelCollection.joins(`inner join \`${ModelThrough.table}\` on \`${ModelThrough.table}\`.\`${options.foreignKey}\` = \`${ModelCollection.table}\`.\`${ModelCollection._primaryKey}\``).where(`\`${ModelThrough.table}\`.\`${throughOptions.foreignKey}\` = ?`, this[primaryKey])
        }
			} else {
