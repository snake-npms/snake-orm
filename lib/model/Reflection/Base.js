...
class Base {
	...
	* */
	...
	async find (valueOfPrimaryKey) {
		let primaryKey = this.Model.getPrimaryKey()
		if (!primaryKey) {
	...
	distinct () {
		return this._inNewReflectionDo((newRelation) => {
			newRelation._sqlFormatter.sqlClause.distinct = true
		})
	}
	
	limit (limit) {
		return this._inNewReflectionDo((newRelation) => {
			newRelation._sqlFormatter.sqlClause.limit = limit - 0
		})
	}
	
	offset (offset) {
		return this._inNewReflectionDo((newRelation) => {
			newRelation._sqlFormatter.sqlClause.offset = offset - 0
		})
	}
	...
	paginate (page = 1, perPage = 20) {
		...
		return this._inNewReflectionDo((newRelation) => {
			newRelation._sqlFormatter.sqlClause.limit = perPage - 0
	...
	async findOrCreateBy (object, blockAfn) {
		return this.Model.withTransaction(async () => {
			let model = await this.findBy(object)
			if (model) {
				return model
			}
	...
	* */
	async updateAll (object) {
		if (!Object.isObject(object)) {
			throw new Error('updateAll params must an object')
		}
	...
	*
	* */
	async destroy (options, ...args) {
		let model = await this.findBy(options, ...args)
		await await model.destroy()
	}
	
	async destroyAll () {
		return this.runSql(this._sqlFormatter.toSql({opDelete: true}))
	}
	
	async destroyAllTableData () {
		return this.runSql(this._sqlFormatter.toSql({opDeleteAll: true}))
	}
	
	joins (options, ...args) {
		return this._inNewReflectionDo((newRelation) => {
			newRelation._sqlFormatter.addJoinClause(options, ...args)
		})
	}
}
module.exports = Base