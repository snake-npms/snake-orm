	...
	addGroupClause (clause) {
		for (let group of clause) {
			if (RegExp(this.Model.table).test(group)) {
				...
				this.sqlClause.groupClause.push(`\`${this.Model.table}\`.\`${group}\``)
			}
		}
	}
		...
		if (typeof clause === 'string') {
			...
			for (let key in clause) {
				if (clause[key] === null) { // is null
					this.sqlClause.havingClause.push(`\`${this.Model.table}\`.\`${key}\` IS NULL`)
				} else {
					let fullKey = `\`${this.Model.table}\`.\`${key}\``
					if (Array.isArray(clause[key])) { // in
						...
						this.sqlClause.havingClause.push(`${fullKey} = ${this.constructor.getSqlValue(clause[key])}`)
					}
				}
			}
		}
