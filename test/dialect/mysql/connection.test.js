...
describe('Mysql connect TEST', function() {
	before(async function() {
		...
		let snakeOrmProxy = await SnakeOrm.initWithSnakeOrmRcFile(path.resolve(__dirname, 'support/.snakeormrc'), async function (snakeOrmProxy) {
      // 在本区块的所有测试用例之前执行
      ...
      await snakeOrmProxy.runSql(`truncate TABLE wallets`)
      await snakeOrmProxy.runSql(`truncate TABLE orders`)
      await snakeOrmProxy.runSql(`truncate TABLE order_items`)
